...
void Util::SmoothDrawQuad(GLfloat* a, GLfloat* b, GLfloat* c, GLfloat* d, floatUV* aUV, floatUV* bUV, floatUV* cUV, floatUV* dUV)
{

	glBegin(GL_QUAD_STRIP);
	glNormal3fv(a);
	glTexCoord2f(aUV->V, aUV->U);
	glVertex3fv(a);

	glNormal3fv(b);
	glTexCoord2f(bUV->V, bUV->U);
	glVertex3fv(b);

	glNormal3fv(c);
	glTexCoord2f(cUV->V, cUV->U);
	glVertex3fv(c);

	glNormal3fv(d);
	glTexCoord2f(dUV->V, dUV->U);
	glVertex3fv(d);
